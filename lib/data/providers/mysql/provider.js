...
MigrationRepository.prototype.dumpSchema = function(destination){
	...
	var def = q.defer();
	try {
		var cmd = 'mysqldump -d -u'+this.config.user+' -p'+this.config.password + ' ' + this.config.database + " | sed 's/ AUTO_INCREMENT=[0-9]*//g' | sed 's/^-- Host:.*$//' | sed 's/Dump completed on .*$//' > " + destination;
		//console.log(cmd)

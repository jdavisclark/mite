...
Mite.prototype.status = function (diskMigrations, opts) {
	...
	return q.when(opts.init || this._requireInit()).then(function () {
		return self.migrationRepo.all(opts.submodule).then(function (dbMigrations) {
          ...
          db.explanation= "In database but not found on disk.";
					dirty.push(db);
				} else if(disk.hash !== db.hash) {
          disk.explanation= "The disk file hash cannot be found in database.";
					dirty.push(disk);
				}
			});
			...
			if (dirty.length > 0) {
				result.dirtyMigrations = dirty.map(function (x) {
					return {key:x.key, hash: x.hash, explanation: x.explanation};
  ...
  var localDebug= false;
	...
	return q.when(opts.init || self._requireInit()).then(function () {
		return q.when(opts.status || self.status(diskMigrations, _.extend(opts, {init: true}))).then(function (miteStatus) {
			...
			} else if (miteStatus.dirtyMigrations && !opts.force) {
				return {
					updated: false,
        ...
        var someExecuted= _.some(miteStatus.executedMigrations,function(key){
          return (miteStatus.unexecutedMigrations && key > miteStatus.unexecutedMigrations[0] && key < miteStatus.unexecutedMigrations[miteStatus.unexecutedMigrations.length - 1]) || (miteStatus.unexecutedMigrations && key > miteStatus.unexecutedMigrations[miteStatus.unexecutedMigrations.length - 1])
        });
        if(localDebug){
	...
	return q.when(opts.init || self._requireInit()).then(function() {
      ...
      var head = executed[executed.length - 1];
			...
			if (direction === "up") {
				...
				if (mStatus.dirtyMigrations && !opts.force) {
					return {
						updated: false,
        ...
        // Report problems which we are ignoring.
				_.each(mStatus.dirtyMigrations,function(dm){
					reporter.warn(util.format("    ignored: key=%s hash=%s explanation=%s",dm.key,dm.hash,dm.explanation))
				});

...
Mite.prototype.up = function (diskMigrations, init, stat) {
	...
	return q.when(init || self._requireInit()).then(function () {
		return q.when(stat || self.status(diskMigrations)).then(function (miteStatus) {
			if (miteStatus.clean) {
				return {
					updated: false,
					wasClean: true
				...
				return self.stepTo(diskMigrations, target);
			}
		});
	});
};
...
Mite.prototype.stepTo = function(diskMigrations, target, init, stat) {
	...
	return q.when(init || self._requireInit()).then(function() {
		...
		return q.when(stat || self.status(diskMigrations)).then(function(mStatus) {
			var executed = mStatus.executedMigrations,
				head = executed[executed.length - 1];

			// up
			if (!head || (!bottom && head.key < target.key)) {
				...
				return upMigrations.reduce(function(p, m) {
					return p.then(self.migrationRepo.executeUpMigration.bind(self.migrationRepo, m));
				}, q()).then(function() {
					return {
						updated: true
					};
				...
				if (missingDowns.length > 0) {
					return {
						...
						missingDown: true,
						migrationsInPathWithoutDown: missingDowns
					};
				}

				// execute the downs serially
				return downMigrations.reduce(function(p, m) {
					return p.then(self.migrationRepo.executeDownMigration.bind(self.migrationRepo, m));
				}, q()).then(function() {
					return {
						updated: true
					};

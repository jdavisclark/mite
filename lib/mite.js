...
Mite.prototype.dumpSchema = function(destination){
	destination = destination || "export.sql"
		...
		return self.status(diskMigrations).then(function(miteStatus) {
			if((miteStatus.clean && diskMigrations.length === 0)	|| (miteStatus.unexecutedMigrations && miteStatus.unexecutedMigrations.length === diskMigrations.length)) {
				...
				if(miteStatus.clean) {
					...
					var head = miteStatus.executedMigrations[miteStatus.executedMigrations.length - 1];
					targetMigration = diskMigrations.filter(function(m) {
						return m.key === head.key;
					})[0];
				}

				// cant execute a down if there is no down...
				if(!targetMigration.down || targetMigration.down.length === 0) {
					return {
						updated: false,
						missingDown: true,
						migrationKey: targetMigration.key
					};
				}
